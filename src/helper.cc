...
bool Helper::OpsWriteStr(const Handle<Object> &obj, DataKeyOperate *dataOps)
{
  ...
  if (!as_operations_add_write_str(&dataOps->ops,
                                    (dataOps->strings.rbegin()+1)->c_str(),
                                    dataOps->strings.back().c_str()))
  {
    ThrowException(Exception::TypeError(String::New("Unknown error in internal operation (as_operations_add_write_str)")));
    return false;
  }
...
bool Helper::OpsWriteInt64(const Handle<Object> &obj, DataKeyOperate *dataOps)
{
  ...
  if (!as_operations_add_write_int64(&dataOps->ops,
                              dataOps->strings.back().c_str(),
                              valueVal->Int32Value()))
  {
    ThrowException(Exception::TypeError(String::New("Unknown error in internal operation (as_operations_add_write_int64)")));
    return false;
  }
